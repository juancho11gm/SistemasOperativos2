...
#define MAX 10000
//estructura que guarda los intervalos de las filas que multiplicara cada proceso
struct x{
	int inicioFila;
	int finFila;
};
...
void arreglarArchivo(char* texEntrada,char* texSalida,int n){
	int matriz[n][n];
	printf("\n ENTRADA: %s     Salida: %s  \n",texEntrada,texSalida);
	FILE *fp = fopen(texEntrada, "r");//abre el archivo
	FILE *fpE = fopen(texSalida, "w");
	char linea[10000],buffer [10000];
	char *token;
	int i,j;
	if(fp){
		for(i=0;i<n;i++){
